...
(function () {
  ...
  style.textContent = [
    ...
    /* ── Homepage hero: hide header initially (prevents FOUC flash) ── */
    /* app.js immediately adds hfg-hero-mode which controls visibility via its own rules. */
    /* This rule only applies until app.js stylesheet (#hfg-anim-styles) is injected.    */
    '.home-page header{opacity:0;pointer-events:none;}',
  ...
  // Called directly by injectNavbar() after the header HTML is inserted.
  // Exposed on window so injectNavbar() (defined later in this file) can call it.
  window._hfgAttachMobileMenu = function attachMobileMenu() {
...
function _hfgToast(msg, type) {
  ...
  var accentColor = isSuccess ? '#22c55e' : '#ef4444'; // green for success, red for error
...
function injectNavbar(activePage) {
  const pages = [
    { key: 'home',     i18nKey: 'nav.home',     label: 'Home',       href: '/' },
    { key: 'samples',  i18nKey: 'nav.products',  label: 'Products',   href: '/products/' },
    { key: 'bulk',     i18nKey: 'nav.inquiry',   label: 'Inquiry',    href: '/inquiry/' },
    { key: 'about',    i18nKey: 'nav.about',     label: 'About Us',   href: '/about/' },
    { key: 'contact',  i18nKey: 'nav.contact',   label: 'Contact',    href: '/contact/' }
  ];

  // Desktop links — active gets red text + red underline, others are dark gray
  const dLinks = pages.map(p => {
    if (p.key === activePage) {
      return `<a href="${p.href}" data-i18n="${p.i18nKey}" style="color:#E10600;border-bottom:2px solid #E10600;padding-bottom:3px;font-size:13px;letter-spacing:0.12em;font-weight:700;text-transform:uppercase;text-decoration:none;white-space:nowrap">${p.label}</a>`;
    }
    return `<a href="${p.href}" data-i18n="${p.i18nKey}" style="color:#1b1b1b;font-size:13px;letter-spacing:0.12em;font-weight:600;text-transform:uppercase;text-decoration:none;white-space:nowrap" onmouseover="this.style.color='#E10600'" onmouseout="this.style.color='#1b1b1b'">${p.label}</a>`;
  }).join('');

  // Mobile links — vertical list with left-border active indicator
  const mLinks = pages.map(p => {
    ...
    return `<a href="${p.href}" data-i18n="${p.i18nKey}" style="${activeStyle};font-size:15px;letter-spacing:0.1em;text-transform:uppercase;text-decoration:none;display:flex;align-items:center;min-height:44px;padding-top:12px;padding-bottom:12px">${p.label}</a>`;
  }).join('');
...
<header style="position:fixed;width:100%;top:0;z-index:50;background:#ffffff;border-bottom:2px solid #1b1b1b">
  <nav style="display:flex;align-items:center;justify-content:space-between;padding:0 16px;max-width:1440px;margin:0 auto;height:72px">
    ...
    <!-- Logo -->
    <a href="/" style="text-decoration:none;flex-shrink:0;display:flex;align-items:center;height:72px" class="hfg-logo">
      <img
    ...
    <div style="position:absolute;left:50%;transform:translateX(-50%);display:flex;align-items:center;gap:28px" class="hfg-desktop-nav">
      ${dLinks}
    </div>

    <!-- Right: lang switcher + mobile toggle -->
    <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
      <div id="hfg-lang-switcher" style="${activePage === 'home' ? '' : 'display:flex;align-items:center'}" class="${activePage === 'home' ? 'hfg-lang-switcher hfg-lang-home' : 'hfg-lang-switcher'}"></div>
...
<style>
  ...
  @media (max-width: 767px) {
    ...
    .hfg-lang-home   { display: none !important; }
    .hfg-logo-img { height: 150px !important; }
  }
</style>`;
  ...
  // Initialize mobile menu immediately — no polling needed
  if (typeof window._hfgAttachMobileMenu === 'function') {
    window._hfgAttachMobileMenu();
  }

  // Hide entire header on mobile home page
  if (activePage === 'home' && window.innerWidth <= 767) {
    var hdr = document.querySelector('header');
    if (hdr) hdr.style.setProperty('display', 'none', 'important');
  }

  // Inject language switcher — try immediately, then retry until HFG_I18N loads
  function tryInjectSwitcher() {
    var slot = document.getElementById('hfg-lang-switcher');
    if (slot && window.HFG_I18N) {
      if (slot.classList.contains('hfg-lang-home') && window.innerWidth <= 767) return true;
      slot.innerHTML = window.HFG_I18N.buildSwitcher();
      return true;
    }
    return false;
  }

  // Try immediately, then keep retrying until ready (max 3s)
  if (!tryInjectSwitcher()) {
    var attempts = 0;
    var poll = setInterval(function() {
      attempts++;
      if (tryInjectSwitcher() || attempts > 60) clearInterval(poll);
    }, 50);
  }
}
...
<footer class="w-full bg-primary border-t-4 border-accent-red">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 px-5 md:px-[64px] py-[80px] max-w-[1440px] mx-auto text-on-primary">
    ...
    <div>
      ...
      <p class="text-primary-fixed-dim font-body-md opacity-80 mb-6 max-w-xs" data-i18n="footer.tagline">
    ...
    <div class="hfg-footer-section">
      <h5 class="hfg-footer-title font-label-bold uppercase text-accent-red">
        <span data-i18n="footer.navigate">Navigate</span>
      ...
      <div class="hfg-footer-content">
        <ul class="flex flex-col gap-3">
          <li><a href="/"               class="text-primary-fixed-dim opacity-80 hover:text-accent-red transition-colors font-body-md" data-i18n="nav.home">Home</a></li>
          <li><a href="/products/"      class="text-primary-fixed-dim opacity-80 hover:text-accent-red transition-colors font-body-md" data-i18n="nav.products">Products</a></li>
          <li><a href="/inquiry/"       class="text-primary-fixed-dim opacity-80 hover:text-accent-red transition-colors font-body-md" data-i18n="nav.inquiry">Inquiry</a></li>
          <li><a href="/about/"         class="text-primary-fixed-dim opacity-80 hover:text-accent-red transition-colors font-body-md" data-i18n="nav.about">About Us</a></li>
          <li><a href="/contact/"       class="text-primary-fixed-dim opacity-80 hover:text-accent-red transition-colors font-body-md" data-i18n="nav.contact">Contact</a></li>
        </ul>
      </div>
    </div>
    ...
    <div class="hfg-footer-section">
      ...
      <div class="hfg-footer-content">
        <ul class="flex flex-col gap-3">
          <li><a href="/privacy-policy/" class="text-primary-fixed-dim opacity-80 hover:text-accent-red transition-colors font-body-md" data-i18n="footer.privacy">Privacy Policy</a></li>
          <li><a href="/terms/"          class="text-primary-fixed-dim opacity-80 hover:text-accent-red transition-colors font-body-md" data-i18n="footer.terms">Terms &amp; Conditions</a></li>
          <li><a href="/shipping/"       class="text-primary-fixed-dim opacity-80 hover:text-accent-red transition-colors font-body-md" data-i18n="footer.shipping">Shipping Info</a></li>
          <li><a href="/contact/"        class="text-primary-fixed-dim opacity-80 hover:text-accent-red transition-colors font-body-md" data-i18n="footer.contact">Contact</a></li>
        </ul>
      </div>
    </div>
    ...
    <div class="hfg-footer-section">
      <h5 class="hfg-footer-title font-label-bold uppercase text-accent-red">
        <span data-i18n="footer.newsletter">Newsletter</span>
        <svg class="hfg-footer-arrow" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0;opacity:0.7;transition:transform 0.3s ease"><path d="M6 9L12 15L18 9" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </h5>
      <div class="hfg-footer-content">
        <div>
          <p class="text-primary-fixed-dim font-body-md opacity-80 mb-5" data-i18n="footer.newsletter.sub">Get updates on new products and offers.</p>
          <div class="flex flex-col gap-2">
            <input
              id="newsletter-email"
              type="email"
              placeholder="EMAIL ADDRESS"
              data-i18n-placeholder="footer.email.placeholder"
  ...
  <div class="border-t border-on-primary/10 py-2 px-2 md:px-[64px]">
    <div class="max-w-[1440px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
      <div class="text-primary-fixed-dim opacity-40 font-body-md text-sm" data-i18n="footer.copyright">
  ...
  (function initFooterAccordion() {
    ...
    document.addEventListener('click', function (e) {
      ...
      // Stop the page-transition from intercepting this click
...
// IMPORTANT: Do NOT initialize INQUIRY_API_URL at parse time.
// config.js may not be loaded yet when app.js first executes on product pages.
// Instead, resolve the URL lazily at the moment of each fetch call.
var _INQUIRY_API_FALLBACK = 'https://script.google.com/macros/s/AKfycbzoC1eoDKPJQTlOe4gZn9bv03lVNBOFoZYNWiZiyNMAmUIiA3xWqpS-Agr6NZRidMurTg/exec';
function _getInquiryApiUrl() {
  return (window.getHFGConfig && window.getHFGConfig('gas.inquiryUrl')) || _INQUIRY_API_FALLBACK;
}
...
function submitInquiryToGoogleSheets() {
  ...
  var clientRefId = 'CR-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8).toUpperCase();
  ...
  fd.append('clientRefId', clientRefId);
  ...
  // Resolve the GAS URL at call time (config.js is loaded by now)
  var apiUrl = _getInquiryApiUrl();
  ...
  fetch(apiUrl, { method: 'POST', body: fd })
    ...
    .then(function (data) {
      if (!data || data.ok === false) {
        var errMsg = (data && data.error) ? data.error : 'Submission failed. Please try again.';
        console.error('[inquiry] GAS error:', errMsg);
        resetBtn();
        _hfgToast(errMsg, 'error');
        return;
      }
      _trackEvent('inquiry_submit', { event_category: 'engagement', event_label: prod, value: parseInt(qty) || 1 });
      if (data.emailWarning) {
        _hfgToast('Inquiry saved. Confirmation email could not be sent — our team will follow up directly.', 'success');
      } else {
        _hfgToast('Inquiry submitted! Check your email for confirmation.', 'success');
      }
...
async function renderProductGrid(container, filter) {
  ...
  container.innerHTML = list.map(p => {
    ...
    return `<a href="${target}" class="group border border-outline-variant bg-surface-container-lowest overflow-hidden flex flex-col" data-reveal style="text-decoration:none;color:inherit;display:flex">
      <div style="width:100%;background:#fff;display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;aspect-ratio:1/1;border-bottom:1px solid #eee">
        <img src="${img}" alt="${p.name}" loading="lazy" class="transition-transform duration-500 group-hover:scale-105" style="display:block;width:100%;height:auto;object-fit:contain;padding:8px" onerror="this.onerror=null;this.src='data:image/svg+xml,%3Csvg xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22 width%3D%22400%22 height%3D%22400%22%3E%3Crect width%3D%22400%22 height%3D%22400%22 fill%3D%22%23eeeeee%22%2F%3E%3C%2Fsvg%3E'"/>
      </div>
      <div class="p-5 flex flex-col flex-grow">
        ...
        <div class="mt-auto">
          <button style="width:100%;background:#E10600;color:#fff;border:none;padding:12px 24px;font-family:'Hanken Grotesk',sans-serif;font-size:14px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;transition:background 0.2s" onclick="window.location.href='${target}'">
            ► SEND INQUIRY
          </button>
        </div>
      </div>
...
(function () {
  ...
  function initNavbarScroll() {
    // Cache header once — fixed in DOM after injectNavbar() runs
  ...
  function initCategoryBarScroll() {
    ...
    // Cache bar position once at init - does not change after render
    const barTop = wrap.getBoundingClientRect().top + window.scrollY;
  ...
  function initButtons() {
    ...
    applyToButtons();
    // Watch only the dynamic grids, not the whole body
    ['#samples-grid', '#featured-grid', '#product-grid'].forEach(function(sel) {
      var g = document.querySelector(sel);
      if (g) new MutationObserver(applyToButtons).observe(g, { childList: true });
    });
  }
  ...
  function initStaggeredGrid() {
    ...
    observeGrids();
    // Watch only the known grid containers, not the whole body
    ['#samples-grid', '#featured-grid'].forEach(function(sel) {
      var g = document.querySelector(sel);
      if (g && !g.dataset.hfgGridObs) {
        g.dataset.hfgGridObs = '1';
        gridObs.observe(g);
        new MutationObserver(observeGrids).observe(g, { childList: true });
      }
    });
  }
