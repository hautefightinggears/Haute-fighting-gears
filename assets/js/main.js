...
function subscribeNewsletter() {
  ...
  if (!emailRegex.test(email)) { alert('Please enter a valid email address.'); if (el) el.focus(); return; }
...
function sendMessage() {
  ...
  if (submitBtn) { submitBtn.disabled = true; submitBtn.innerHTML = '<span class="material-symbols-outlined text-[18px]">hourglass_empty</span> Sending...'; }
  ...
  fetch(HFG_FORMS_URL, { method: 'POST', body: fd })
    ...
    .catch(function (err) {
      ...
      reset();
      alert('Could not connect. Please check your connection or contact us via WhatsApp.');
    });
}
...
function submitInquiryToGoogleSheets() {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { alert('Please enter a valid email address.'); emailEl.focus(); return; }
    if (!phone) { alert('Please enter your phone number.'); phoneEl.focus(); return; }
    if (!prod) { alert('Please enter the product name.'); prodEl.focus(); return; }
    if (!qty || parseInt(qty) < 1) { alert('Please enter a valid quantity.'); qtyEl.focus(); return; }
    ...
    fetch(INQUIRY_API_URL, { method: 'POST', body: fd })
        .then(function () {
            alert('Inquiry submitted successfully! We\'ll respond within 24 hours.');
        ...
        .catch(function (err) {
            ...
            alert('Could not connect. Please try WhatsApp instead.');
        });
}
...
function submitInquiryAsWhatsApp() {
    ...
    if (!name || !email || !phone || !prod || !qty) { alert('Please fill in all required fields.'); return; }
