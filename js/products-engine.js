...
async function renderProductGrid(container, filter) {
    ...
    container.innerHTML = list.map(p => {
        ...
        return `
        <a href="${pagePrefix}product.html?id=${encodeURIComponent(p.id)}" class="group border border-outline-variant bg-surface-container-lowest overflow-hidden flex flex-col" data-reveal style="text-decoration:none;color:inherit;display:flex">
            <div class="relative aspect-square overflow-hidden block bg-white">
                <img
                    src="${img}"
                    alt="${p.name}"
                    loading="lazy"
            ...
            <div class="p-5 flex flex-col flex-grow">
                ...
                <div class="mt-auto flex flex-col gap-2">
                    <span class="w-full bg-accent-red text-on-primary py-3 font-label-bold uppercase flex items-center justify-center gap-2 hover:bg-primary transition-colors text-center text-sm" style="pointer-events:none">
